...
public class WeatherRetrofitManager {
    ...
    private static WeatherRetrofitManager instance = null;

    public static final String BASE_URL_PRODUCTION = "http://api.openweathermap.org/data/2.5/"; //release server
    public static final String BASE_URL_STAGE = "http://api.openweathermap.org/data/2.5/"; //development server

    private Retrofit retrofit;

    private WeatherRetrofitManager() {
    }

    public static WeatherRetrofitManager getInstance() {
        if (instance == null) {
            synchronized (WeatherRetrofitManager.class) {
                if (instance == null) {
                    instance = new WeatherRetrofitManager();
                }
            }
        }
        return instance;
